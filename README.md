...
This is now a **two-step process**: jlink first (bundles a minimal Java
runtime containing exactly the modules you need), then jpackage (wraps that
into a native Windows installer).

```bash
:: Step 1: jlink — build a custom runtime image
jlink --module-path "target/modules;%JAVA_HOME%\jmods" ^
  --add-modules site.kpeclub.launcher,javafx.controls,javafx.fxml ^
  --output target/runtime ^
  --strip-debug --compress=2 --no-header-files --no-man-pages

:: Step 2: jpackage — wrap the runtime image into an installer
jpackage --type exe ^
  --name "KPE Club Launcher" ^
  --app-version 1.0.0 ^
  --vendor "KPE Club" ^
  --description "KPE Club's Minecraft launcher" ^
  --runtime-image target/runtime ^
  --module site.kpeclub.launcher/site.kpeclub.launcher.Main ^
  --win-shortcut --win-menu ^
  --icon src/main/resources/images/icon.ico
```

Requires **WiX Toolset** installed for the `--type exe` installer format:
```
winget install WiXToolset.WiXToolset
```

### Uninstalling
jpackage's `--type exe` installer **already includes a full uninstaller** —
this is automatic, no extra setup needed. Players can remove KPE Club
Launcher the normal Windows way:
- **Settings → Apps → Installed apps** → search "KPE Club Launcher" → Uninstall
- or **Control Panel → Programs and Features** → same thing

The `--vendor` and `--description` flags above just make that listing show
"KPE Club" and a clear description instead of generic defaults. Uninstalling
removes the app files and Start Menu/desktop shortcuts, but does **not**
touch `%APPDATA%\.kpelauncher\` or `%APPDATA%\.minecraft\` — a player's
downloaded Minecraft versions, mods, and worlds are left untouched, same as
how the official Minecraft Launcher behaves.
